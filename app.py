...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://nyameget:77Gsoy3wKIUf96VTGcp8rbdjlPsKeFqx@dpg-cq1ed02j1k6c73bstmig-a.oregon-postgres.render.com/final_project_t2zo'
    ...
    try:
        if request.method == 'GET':
            ...
            if choice == 'password':
                ...
                passwordConfirmation = str(request.form['newPasswordConfirmation'])
                ...
                oldPassword = str(request.form['oldPassword'])
                oldEmail = str(request.form['oldEmail']).lower()
                newEmail = str(request.form['newEmail']).lower()
...
def review():
    try:
        ...
        if 'username' in session:
            ...
            flash('An error occured please try again', 'danger')
            return redirect(url_for('setting'))
        # Review Encountered an error and user is not logged in
